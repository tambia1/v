import iconActivity from "./assets/activity.svg";
import iconAirplay from "./assets/airplay.svg";
import iconAlertCircle from "./assets/alert-circle.svg";
import iconAlertOctagon from "./assets/alert-octagon.svg";
import iconAlertTriangle from "./assets/alert-triangle.svg";
import iconAlignCenter from "./assets/align-center.svg";
import iconAlignJustify from "./assets/align-justify.svg";
import iconAlignLeft from "./assets/align-left.svg";
import iconAlignRight from "./assets/align-right.svg";
import iconAmazon from "./assets/amazon.svg";
import iconAnchor from "./assets/anchor.svg";
import iconAperture from "./assets/aperture.svg";
import iconApple from "./assets/apple.svg";
import iconArchive from "./assets/archive.svg";
import iconArrowDownCircle from "./assets/arrow-down-circle.svg";
import iconArrowDownLeft from "./assets/arrow-down-left.svg";
import iconArrowDownRight from "./assets/arrow-down-right.svg";
import iconArrowDown from "./assets/arrow-down.svg";
import iconArrowLeftCircle from "./assets/arrow-left-circle.svg";
import iconArrowLeft from "./assets/arrow-left.svg";
import iconArrowRightCircle from "./assets/arrow-right-circle.svg";
import iconArrowRight from "./assets/arrow-right.svg";
import iconArrowUpCircle from "./assets/arrow-up-circle.svg";
import iconArrowUpLeft from "./assets/arrow-up-left.svg";
import iconArrowUpRight from "./assets/arrow-up-right.svg";
import iconArrowUp from "./assets/arrow-up.svg";
import iconAtSign from "./assets/at-sign.svg";
import iconAward from "./assets/award.svg";
import iconBarChart2 from "./assets/bar-chart-2.svg";
import iconBarChart from "./assets/bar-chart.svg";
import iconBatteryCharging from "./assets/battery-charging.svg";
import iconBattery from "./assets/battery.svg";
import iconBellOff from "./assets/bell-off.svg";
import iconBell from "./assets/bell.svg";
import iconBluetooth from "./assets/bluetooth.svg";
import iconBold from "./assets/bold.svg";
import iconBookOpen from "./assets/book-open.svg";
import iconBook from "./assets/book.svg";
import iconBookmark from "./assets/bookmark.svg";
import iconBox from "./assets/box.svg";
import iconBriefcase from "./assets/briefcase.svg";
import iconCalendar from "./assets/calendar.svg";
import iconCameraOff from "./assets/camera-off.svg";
import iconCamera from "./assets/camera.svg";
import iconCast from "./assets/cast.svg";
import iconCheckCircle from "./assets/check-circle.svg";
import iconCheckSquare from "./assets/check-square.svg";
import iconCheck from "./assets/check.svg";
import iconChevronDown from "./assets/chevron-down.svg";
import iconChevronLeft from "./assets/chevron-left.svg";
import iconChevronRight from "./assets/chevron-right.svg";
import iconChevronUp from "./assets/chevron-up.svg";
import iconChevronsDown from "./assets/chevrons-down.svg";
import iconChevronsLeft from "./assets/chevrons-left.svg";
import iconChevronsRight from "./assets/chevrons-right.svg";
import iconChevronsUp from "./assets/chevrons-up.svg";
import iconChrome from "./assets/chrome.svg";
import iconCircle from "./assets/circle.svg";
import iconClipboard from "./assets/clipboard.svg";
import iconClock from "./assets/clock.svg";
import iconCloudDrizzle from "./assets/cloud-drizzle.svg";
import iconCloudLightning from "./assets/cloud-lightning.svg";
import iconCloudOff from "./assets/cloud-off.svg";
import iconCloudRain from "./assets/cloud-rain.svg";
import iconCloudSnow from "./assets/cloud-snow.svg";
import iconCloud from "./assets/cloud.svg";
import iconCode from "./assets/code.svg";
import iconCodepen from "./assets/codepen.svg";
import iconCodesandbox from "./assets/codesandbox.svg";
import iconCoffee from "./assets/coffee.svg";
import iconColumns from "./assets/columns.svg";
import iconCommand from "./assets/command.svg";
import iconCompass from "./assets/compass.svg";
import iconCopy from "./assets/copy.svg";
import iconCornerDownLeft from "./assets/corner-down-left.svg";
import iconCornerDownRight from "./assets/corner-down-right.svg";
import iconCornerLeftDown from "./assets/corner-left-down.svg";
import iconCornerLeftUp from "./assets/corner-left-up.svg";
import iconCornerRightDown from "./assets/corner-right-down.svg";
import iconCornerRightUp from "./assets/corner-right-up.svg";
import iconCornerUpLeft from "./assets/corner-up-left.svg";
import iconCornerUpRight from "./assets/corner-up-right.svg";
import iconCpu from "./assets/cpu.svg";
import iconCreditCard from "./assets/credit-card.svg";
import iconCrop from "./assets/crop.svg";
import iconCrosshair from "./assets/crosshair.svg";
import iconDatabase from "./assets/database.svg";
import iconDelete from "./assets/delete.svg";
import iconDisc from "./assets/disc.svg";
import iconDivideCircle from "./assets/divide-circle.svg";
import iconDivideSquare from "./assets/divide-square.svg";
import iconDivide from "./assets/divide.svg";
import iconDollarSign from "./assets/dollar-sign.svg";
import iconDownloadCloud from "./assets/download-cloud.svg";
import iconDownload from "./assets/download.svg";
import iconDribbble from "./assets/dribbble.svg";
import iconDroplet from "./assets/droplet.svg";
import iconEdit2 from "./assets/edit-2.svg";
import iconEdit3 from "./assets/edit-3.svg";
import iconEdit from "./assets/edit.svg";
import iconExternalLink from "./assets/external-link.svg";
import iconEyeOff from "./assets/eye-off.svg";
import iconEye from "./assets/eye.svg";
import iconFacebook from "./assets/facebook.svg";
import iconFastForward from "./assets/fast-forward.svg";
import iconFeather from "./assets/feather.svg";
import iconFigma from "./assets/figma.svg";
import iconFileMinus from "./assets/file-minus.svg";
import iconFilePlus from "./assets/file-plus.svg";
import iconFileText from "./assets/file-text.svg";
import iconFile from "./assets/file.svg";
import iconFilm from "./assets/film.svg";
import iconFilter from "./assets/filter.svg";
import iconFlag from "./assets/flag.svg";
import iconFolderMinus from "./assets/folder-minus.svg";
import iconFolderPlus from "./assets/folder-plus.svg";
import iconFolder from "./assets/folder.svg";
import iconFramer from "./assets/framer.svg";
import iconFrown from "./assets/frown.svg";
import iconGift from "./assets/gift.svg";
import iconGitBranch from "./assets/git-branch.svg";
import iconGitCommit from "./assets/git-commit.svg";
import iconGitMerge from "./assets/git-merge.svg";
import iconGitPullRequest from "./assets/git-pull-request.svg";
import iconGithub from "./assets/github.svg";
import iconGitlab from "./assets/gitlab.svg";
import iconGlobe from "./assets/globe.svg";
import iconGoogle from "./assets/google.svg";
import iconGrid from "./assets/grid.svg";
import iconHardDrive from "./assets/hard-drive.svg";
import iconHash from "./assets/hash.svg";
import iconHeadphones from "./assets/headphones.svg";
import iconHeart from "./assets/heart.svg";
import iconHelpCircle from "./assets/help-circle.svg";
import iconHexagon from "./assets/hexagon.svg";
import iconHome from "./assets/home.svg";
import iconImage from "./assets/image.svg";
import iconInbox from "./assets/inbox.svg";
import iconInfo from "./assets/info.svg";
import iconInstagram from "./assets/instagram.svg";
import iconItalic from "./assets/italic.svg";
import iconKey from "./assets/key.svg";
import iconLayers from "./assets/layers.svg";
import iconLayout from "./assets/layout.svg";
import iconLifeBuoy from "./assets/life-buoy.svg";
import iconLink2 from "./assets/link-2.svg";
import iconLink from "./assets/link.svg";
import iconLinkedin from "./assets/linkedin.svg";
import iconList from "./assets/list.svg";
import iconLoader from "./assets/loader.svg";
import iconLock from "./assets/lock.svg";
import iconLogIn from "./assets/log-in.svg";
import iconLogOut from "./assets/log-out.svg";
import iconMail from "./assets/mail.svg";
import iconMapPin from "./assets/map-pin.svg";
import iconMap from "./assets/map.svg";
import iconMaximize2 from "./assets/maximize-2.svg";
import iconMaximize from "./assets/maximize.svg";
import iconMeh from "./assets/meh.svg";
import iconMenu from "./assets/menu.svg";
import iconMessageCircle from "./assets/message-circle.svg";
import iconMessageSquare from "./assets/message-square.svg";
import iconMicOff from "./assets/mic-off.svg";
import iconMic from "./assets/mic.svg";
import iconMicrosoft from "./assets/microsoft.svg";
import iconMinimize2 from "./assets/minimize-2.svg";
import iconMinimize from "./assets/minimize.svg";
import iconMinusCircle from "./assets/minus-circle.svg";
import iconMinusSquare from "./assets/minus-square.svg";
import iconMinus from "./assets/minus.svg";
import iconMonitor from "./assets/monitor.svg";
import iconMoon from "./assets/moon.svg";
import iconMoreHorizontal from "./assets/more-horizontal.svg";
import iconMoreVertical from "./assets/more-vertical.svg";
import iconMousePointer from "./assets/mouse-pointer.svg";
import iconMove from "./assets/move.svg";
import iconMusic from "./assets/music.svg";
import iconNavigation2 from "./assets/navigation-2.svg";
import iconNavigation from "./assets/navigation.svg";
import iconOctagon from "./assets/octagon.svg";
import iconPackage from "./assets/package.svg";
import iconPaperclip from "./assets/paperclip.svg";
import iconPauseCircle from "./assets/pause-circle.svg";
import iconPause from "./assets/pause.svg";
import iconPenTool from "./assets/pen-tool.svg";
import iconPercent from "./assets/percent.svg";
import iconPhoneCall from "./assets/phone-call.svg";
import iconPhoneForwarded from "./assets/phone-forwarded.svg";
import iconPhoneIncoming from "./assets/phone-incoming.svg";
import iconPhoneMissed from "./assets/phone-missed.svg";
import iconPhoneOff from "./assets/phone-off.svg";
import iconPhoneOutgoing from "./assets/phone-outgoing.svg";
import iconPhone from "./assets/phone.svg";
import iconPieChart from "./assets/pie-chart.svg";
import iconPlayCircle from "./assets/play-circle.svg";
import iconPlay from "./assets/play.svg";
import iconPlusCircle from "./assets/plus-circle.svg";
import iconPlusSquare from "./assets/plus-square.svg";
import iconPlus from "./assets/plus.svg";
import iconPocket from "./assets/pocket.svg";
import iconPower from "./assets/power.svg";
import iconPrinter from "./assets/printer.svg";
import iconRadio from "./assets/radio.svg";
import iconRedisActivated from "./assets/redis-activated.svg";
import iconRedisDisabled from "./assets/redis-disabaled.svg";
import iconRedisDissolved from "./assets/redis-dissolved.svg";
import iconRedis from "./assets/redis.svg";
import iconRefreshCcw from "./assets/refresh-ccw.svg";
import iconRefreshCw from "./assets/refresh-cw.svg";
import iconRepeat from "./assets/repeat.svg";
import iconRewind from "./assets/rewind.svg";
import iconRotateCcw from "./assets/rotate-ccw.svg";
import iconRotateCw from "./assets/rotate-cw.svg";
import iconRss from "./assets/rss.svg";
import iconSave from "./assets/save.svg";
import iconScissors from "./assets/scissors.svg";
import iconSearch from "./assets/search.svg";
import iconSend from "./assets/send.svg";
import iconServerSingle from "./assets/server-single.svg";
import iconServer from "./assets/server.svg";
import iconSettings from "./assets/settings.svg";
import iconShare2 from "./assets/share-2.svg";
import iconShare from "./assets/share.svg";
import iconShieldOff from "./assets/shield-off.svg";
import iconShield from "./assets/shield.svg";
import iconShoppingBag from "./assets/shopping-bag.svg";
import iconShoppingCart from "./assets/shopping-cart.svg";
import iconShuffle from "./assets/shuffle.svg";
import iconSidebar from "./assets/sidebar.svg";
import iconSkipBack from "./assets/skip-back.svg";
import iconSkipForward from "./assets/skip-forward.svg";
import iconSlack from "./assets/slack.svg";
import iconSlash from "./assets/slash.svg";
import iconSliders from "./assets/sliders.svg";
import iconSmartphone from "./assets/smartphone.svg";
import iconSmile from "./assets/smile.svg";
import iconSpeaker from "./assets/speaker.svg";
import iconSquare from "./assets/square.svg";
import iconStar from "./assets/star.svg";
import iconStopCircle from "./assets/stop-circle.svg";
import iconSun from "./assets/sun.svg";
import iconSunrise from "./assets/sunrise.svg";
import iconSunset from "./assets/sunset.svg";
import iconTable from "./assets/table.svg";
import iconTablet from "./assets/tablet.svg";
import iconTag from "./assets/tag.svg";
import iconTarget from "./assets/target.svg";
import iconTerminal from "./assets/terminal.svg";
import iconThermometer from "./assets/thermometer.svg";
import iconThumbsDown from "./assets/thumbs-down.svg";
import iconThumbsUp from "./assets/thumbs-up.svg";
import iconToggleLeft from "./assets/toggle-left.svg";
import iconToggleRight from "./assets/toggle-right.svg";
import iconTool from "./assets/tool.svg";
import iconTrash2 from "./assets/trash-2.svg";
import iconTrash from "./assets/trash.svg";
import iconTrello from "./assets/trello.svg";
import iconTrendingDown from "./assets/trending-down.svg";
import iconTrendingUp from "./assets/trending-up.svg";
import iconTriangle from "./assets/triangle.svg";
import iconTruck from "./assets/truck.svg";
import iconTv from "./assets/tv.svg";
import iconTwitch from "./assets/twitch.svg";
import iconTwitter from "./assets/twitter.svg";
import iconType from "./assets/type.svg";
import iconUmbrella from "./assets/umbrella.svg";
import iconUnderline from "./assets/underline.svg";
import iconUnlock from "./assets/unlock.svg";
import iconUploadCloud from "./assets/upload-cloud.svg";
import iconUpload from "./assets/upload.svg";
import iconUserCheck from "./assets/user-check.svg";
import iconUserMinus from "./assets/user-minus.svg";
import iconUserPlus from "./assets/user-plus.svg";
import iconUserX from "./assets/user-x.svg";
import iconUser from "./assets/user.svg";
import iconUsers from "./assets/users.svg";
import iconVSquare from "./assets/v-square.svg";
import iconVideoOff from "./assets/video-off.svg";
import iconVideo from "./assets/video.svg";
import iconVoicemail from "./assets/voicemail.svg";
import iconVolume1 from "./assets/volume-1.svg";
import iconVolume2 from "./assets/volume-2.svg";
import iconVolumeX from "./assets/volume-x.svg";
import iconVolume from "./assets/volume.svg";
import iconWatch from "./assets/watch.svg";
import iconWifiOff from "./assets/wifi-off.svg";
import iconWifi from "./assets/wifi.svg";
import iconWind from "./assets/wind.svg";
import iconXCircle from "./assets/x-circle.svg";
import iconXOctagon from "./assets/x-octagon.svg";
import iconXSquare from "./assets/x-square.svg";
import iconX from "./assets/x.svg";
import iconYoutube from "./assets/youtube.svg";
import iconZapOff from "./assets/zap-off.svg";
import iconZap from "./assets/zap.svg";
import iconZoomIn from "./assets/zoom-in.svg";
import iconZoomOut from "./assets/zoom-out.svg";

export const Icons = {
	"": "",
	iconActivity,
	iconAirplay,
	iconAlertCircle,
	iconAlertOctagon,
	iconAlertTriangle,
	iconAlignCenter,
	iconAlignJustify,
	iconAlignLeft,
	iconAlignRight,
	iconAnchor,
	iconAperture,
	iconArchive,
	iconArrowDownCircle,
	iconArrowDownLeft,
	iconArrowDownRight,
	iconArrowDown,
	iconArrowLeftCircle,
	iconArrowLeft,
	iconArrowRightCircle,
	iconArrowRight,
	iconArrowUpCircle,
	iconArrowUpLeft,
	iconArrowUpRight,
	iconArrowUp,
	iconAtSign,
	iconAward,
	iconBarChart2,
	iconBarChart,
	iconBatteryCharging,
	iconBattery,
	iconBellOff,
	iconBell,
	iconBluetooth,
	iconBold,
	iconBookOpen,
	iconBook,
	iconBookmark,
	iconBox,
	iconBriefcase,
	iconCalendar,
	iconCameraOff,
	iconCamera,
	iconCast,
	iconCheckCircle,
	iconCheckSquare,
	iconCheck,
	iconChevronDown,
	iconChevronLeft,
	iconChevronRight,
	iconChevronUp,
	iconChevronsDown,
	iconChevronsLeft,
	iconChevronsRight,
	iconChevronsUp,
	iconChrome,
	iconCircle,
	iconClipboard,
	iconClock,
	iconCloudDrizzle,
	iconCloudLightning,
	iconCloudOff,
	iconCloudRain,
	iconCloudSnow,
	iconCloud,
	iconCode,
	iconCodepen,
	iconCodesandbox,
	iconCoffee,
	iconColumns,
	iconCommand,
	iconCompass,
	iconCopy,
	iconCornerDownLeft,
	iconCornerDownRight,
	iconCornerLeftDown,
	iconCornerLeftUp,
	iconCornerRightDown,
	iconCornerRightUp,
	iconCornerUpLeft,
	iconCornerUpRight,
	iconCpu,
	iconCreditCard,
	iconCrop,
	iconCrosshair,
	iconDatabase,
	iconDelete,
	iconDisc,
	iconDivideCircle,
	iconDivideSquare,
	iconDivide,
	iconDollarSign,
	iconDownloadCloud,
	iconDownload,
	iconDribbble,
	iconDroplet,
	iconEdit2,
	iconEdit3,
	iconEdit,
	iconExternalLink,
	iconEyeOff,
	iconEye,
	iconFacebook,
	iconFastForward,
	iconFeather,
	iconFigma,
	iconFileMinus,
	iconFilePlus,
	iconFileText,
	iconFile,
	iconFilm,
	iconFilter,
	iconFlag,
	iconFolderMinus,
	iconFolderPlus,
	iconFolder,
	iconFramer,
	iconFrown,
	iconGift,
	iconGitBranch,
	iconGitCommit,
	iconGitMerge,
	iconGitPullRequest,
	iconGithub,
	iconGitlab,
	iconGlobe,
	iconGrid,
	iconHardDrive,
	iconHash,
	iconHeadphones,
	iconHeart,
	iconHelpCircle,
	iconHexagon,
	iconHome,
	iconImage,
	iconInbox,
	iconInfo,
	iconInstagram,
	iconItalic,
	iconKey,
	iconLayers,
	iconLayout,
	iconLifeBuoy,
	iconLink2,
	iconLink,
	iconLinkedin,
	iconList,
	iconLoader,
	iconLock,
	iconLogIn,
	iconLogOut,
	iconMail,
	iconMapPin,
	iconMap,
	iconMaximize2,
	iconMaximize,
	iconMeh,
	iconMenu,
	iconMessageCircle,
	iconMessageSquare,
	iconMicOff,
	iconMic,
	iconMinimize2,
	iconMinimize,
	iconMinusCircle,
	iconMinusSquare,
	iconMinus,
	iconMonitor,
	iconMoon,
	iconMoreHorizontal,
	iconMoreVertical,
	iconMousePointer,
	iconMove,
	iconMusic,
	iconNavigation2,
	iconNavigation,
	iconOctagon,
	iconPackage,
	iconPaperclip,
	iconPauseCircle,
	iconPause,
	iconPenTool,
	iconPercent,
	iconPhoneCall,
	iconPhoneForwarded,
	iconPhoneIncoming,
	iconPhoneMissed,
	iconPhoneOff,
	iconPhoneOutgoing,
	iconPhone,
	iconPieChart,
	iconPlayCircle,
	iconPlay,
	iconPlusCircle,
	iconPlusSquare,
	iconPlus,
	iconPocket,
	iconPower,
	iconPrinter,
	iconRadio,
	iconRedisActivated,
	iconRedisDisabled,
	iconRedisDissolved,
	iconRedis,
	iconRefreshCcw,
	iconRefreshCw,
	iconRepeat,
	iconRewind,
	iconRotateCcw,
	iconRotateCw,
	iconRss,
	iconSave,
	iconScissors,
	iconSearch,
	iconSend,
	iconServer,
	iconServerSingle,
	iconSettings,
	iconShare2,
	iconShare,
	iconShieldOff,
	iconShield,
	iconShoppingBag,
	iconShoppingCart,
	iconShuffle,
	iconSidebar,
	iconSkipBack,
	iconSkipForward,
	iconSlack,
	iconSlash,
	iconSliders,
	iconSmartphone,
	iconSmile,
	iconSpeaker,
	iconSquare,
	iconStar,
	iconStopCircle,
	iconSun,
	iconSunrise,
	iconSunset,
	iconTable,
	iconTablet,
	iconTag,
	iconTarget,
	iconTerminal,
	iconThermometer,
	iconThumbsDown,
	iconThumbsUp,
	iconToggleLeft,
	iconToggleRight,
	iconTool,
	iconTrash2,
	iconTrash,
	iconTrello,
	iconTrendingDown,
	iconTrendingUp,
	iconTriangle,
	iconTruck,
	iconTv,
	iconTwitch,
	iconTwitter,
	iconType,
	iconUmbrella,
	iconUnderline,
	iconUnlock,
	iconUploadCloud,
	iconUpload,
	iconUserCheck,
	iconUserMinus,
	iconUserPlus,
	iconUserX,
	iconUser,
	iconUsers,
	iconVideoOff,
	iconVideo,
	iconVoicemail,
	iconVolume1,
	iconVolume2,
	iconVolumeX,
	iconVolume,
	iconVSquare,
	iconWatch,
	iconWifiOff,
	iconWifi,
	iconWind,
	iconXCircle,
	iconXOctagon,
	iconXSquare,
	iconX,
	iconYoutube,
	iconZapOff,
	iconZap,
	iconZoomIn,
	iconZoomOut,
	iconGoogle,
	iconApple,
	iconMicrosoft,
	iconAmazon,
} as const;

export type IIconName = keyof typeof Icons;
